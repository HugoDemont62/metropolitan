...
export default function Paints() {
  ...
  const filteredPaintings = useMemo(() => {
    ...
    return paintings.filter(painting => {
      // Recherche
      if (searchTerm) {
        const matchesTitle = painting.title?.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesArtist = painting.artist?.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesType = painting.type?.toLowerCase().includes(searchTerm.toLowerCase());

        if (!matchesTitle && !matchesArtist && !matchesType) return false;
      }

      // Filtre location
      if (selectedLocation !== 'all' && painting.location !== selectedLocation) {
        return false;
      }

      // Filtre mouvement
      if (selectedMovement !== 'all' && painting.movement !== selectedMovement) {
        return false;
      }

      // Domaine public (si tu veux filtrer autrement, adapte ici)
  ...
  return (
    <main className="px-6 py-18">
      <div className="mb-8">
        <h1 className="text-4xl mb-4 font-title tracking-wider">Nos Peintures</h1>
      ...
      <div className="mb-8 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          {(selectedLocation !== 'all' || selectedMovement !== 'all' || isOnView) && (
            <button
          ...
          <div className="text-sm text-gray-500 ml-auto">
            {filteredPaintings.length} résultat{filteredPaintings.length > 1 ? 's' : ''}
          </div>
        </div>
      </div>
